...
def control_vehicle(vehicle):
    ...
    def on_key_release(key):
        ...
        if key.name == "left":
            steer=0# steer += STEER_INCREMENT
        elif key.name == "right":
            steer=0# steer -= STEER_INCREMENT
        if key.name == "up":
            throttle =0# throttle -= THROTTLE_INCREMENT
        if key.name == "down":
            brake=0# brake -= BRAKE_INCREMENT
...
# Create a function to generate the map image
def generate_map_image(world, width, height):
    # Get the top-down view of the map
    spectator = world.get_spectator()
    spectator_transform = spectator.get_transform()
    spectator_transform.location.z += 200  # Adjust the height for better visualization
    spectator.set_transform(spectator_transform)
    ...
    # Reset the spectator transform
    spectator_transform.location.z -= 200
    spectator.set_transform(spectator_transform)
...
# Create a window to display the map image
cv2.namedWindow("Map", cv2.WINDOW_NORMAL)
cv2.resizeWindow("Map", 800, 600)

# Generate and display the map image
map_image = generate_map_image(world, 800, 600)
